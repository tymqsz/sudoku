...
for row in range(len(df.index)):
    if df.iloc[row]["label"] == 0:
        ...
        i_zero += 1
    if df.iloc[row]["label"] == 4 and i_four < 800:
        df.iloc[row] = fours_s.iloc[i_four].copy()
        i_four += 1
    if df.iloc[row]["label"] == 6 and i_six < 800:
        df.iloc[row] = sixes.iloc[i_six].copy()
        i_six += 1
    if df.iloc[row]["label"] == 8 and i_eight < 800:
        df.iloc[row] = eights.iloc[i_eight].copy()
        i_eight += 1
    if df.iloc[row]["label"] == 1 and i_one < 800:
        df.iloc[row] = ones_s.iloc[i_one].copy()
        i_one += 1
    if df.iloc[row]["label"] == 9 and i_nine < 800:
        df.iloc[row] = nines_s.iloc[i_nine].copy()
        i_nine += 1

i_two, i_six, i_seven = 0, 0, 0
for row in range(len(df.index)):
    if df.iloc[row]["label"] == 6 and i_six < 800:
        df.iloc[row] = six_s.iloc[i_six].copy()
        i_six += 1
    if df.iloc[row]["label"] == 7 and i_seven < 500:
        df.iloc[row] = seven_s.iloc[i_seven].copy()
        i_seven += 1
    if df.iloc[row]["label"] == 2 and i_two < 800:
